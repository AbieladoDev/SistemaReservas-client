...
```
> ⚠️ O nome da pasta **precisa ser `reservas`** (os caminhos e as APIs usam `/reservas/`).
...
### 4. Criar o usuário admin (roda uma vez)
Abra no navegador: **http://localhost/reservas/instalar.php**
Isso cria os logins:
...
| Login não funciona | Abriu `instalar.php` uma vez para criar os usuários? |
